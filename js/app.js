...
function showDirectorReportModal(isSuccess) {
    ...
    if (!modal) {
        ...
        modal.innerHTML = `
        <div class="popup-box">
            <div class="popup-icon" id="drModalIcon"></div>
            <div class="popup-title" id="drModalTitle"></div>
            <div class="popup-msg" id="drModalMsg"></div>
            <button class="popup-close-btn" id="drModalCloseBtn">Done</button>
        </div>
        `;
        ...
        if (isBreakdown && selectedWeekId) {
            ...
            if (allWeeks.length === 0) {
                ...
                allWeeks.forEach(weekId => {
                    ...
                    const submittedCount = weekData.filter(i => (i['Submission Status'] || '').toLowerCase().includes('submitted')).length;
    ...
    mappedData.forEach((data, i) => {
        tableBody.innerHTML += `
            <tr>
                ...
                <td><span class="status ${data.statusClass}">${data.status}</span></td>
            </tr>
        `;
    });
}
